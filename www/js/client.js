...
function loadRemoteMediaStream(stream, peers, peer_id) {
    ...
    console.log(remoteMediaStream.getVideoTracks());
    console.log(remoteMediaStream.getVideoTracks()[0]);
    */
    ...
    startbutton.addEventListener('click', function(ev){
      ...
      function takepicture() {
        ...
        function doSomething(success){
            ...
            $(document).ready(function () {
                $("#submit").click(function () {
                   $.post("/emotionsresult",
                      {
                         result: success["instance_1"]["dominant_emotion"]
                      },
        ...
        /*
        //trying hands on ajax to send the image data as post to our deepface api
        ...
        console.log('makeblob function execution successfully!')
        return new Blob([uInt8Array], { type: contentType });
        
     }//end of makeblob function
     ...
     $.ajax({
        ...
        data: makeblob(image_data_in_json),
     })
    ...
    */
    ...
    function sendJSON(imgdata){           
        ...
        var image_data_in_json = JSON.stringify({"img": [imgdata]});
        console.log(image_data_in_json)

        // Sending data with the request
        xhr.send(image_data_in_json);
    }
    */
...
function startup() {
    ...
    var startbutton = document.getElementById('startbutton');
    //document.getElementById("startbutton").style.display = "none";
    function save_recording_on_load(stream,peer_id){
        stream = stream;
        video = document.getElementById(peer_id+'_'+'video');
        console.log(peer_id+'_'+'video');
    }
    ...
    //this function will click the button every 100 ms
    //setInterval(function () {document.getElementById("startbutton").click();}, 100);
...
//start and stop record remote stream function
/*
///save the recording of RemoteMediaStream
function save_recording_on_load(stream){
    stream =  stream;
    let mimeType = 'video/webm';
    mediaRecorder = createRecorder(stream, mimeType);
    console.log('Remote recording initiated...')
}
...
function createRecorder (stream, mimeType) {
    ...
    //comment savefile function if you just want to share screen , but not save the screenshared.
...
function saveFile(recordedChunks){
    //blob contains all the video data now
    const blob = new Blob(recordedChunks, {
       type: 'video/webm'
     });
     //ask for whether provider want's to generate the report?
     var answer = window.confirm('Generating the report along with Downloading Client Data?');
     if (answer) {
*/
...
/*
        var arrayPromise = new Promise(function(resolve) {
            ...
            reader.onloadend = function() {
                ...
                fs.writeFile('./meet_recordings/name.mp4', buffer, {}, (err, res) => {
                    if(err){
                        console.error(err)
                        return
                    }
                    console.log('video saved using writefile')
                });
            };
...
/*
        ...
        let filename = window.prompt('Enter file name'),
            downloadLink = document.createElement('a');
        downloadLink.href = URL.createObjectURL(blob);
        downloadLink.download = `url('${filename}.mp4')`;
        document.body.appendChild(downloadLink);
        //downloadLink.click();
        console.log(downloadLink)
        console.log(downloadLink.download)
